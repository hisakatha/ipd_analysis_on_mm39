...
parse_kinetics_h5 <- function(file, chromosomes) {
    ...
    foreach_ret <- foreach(chr = chromosomes) %do% {
        ...
        write_fst(kinetics_dt, sprintf("load_and_save.wu2016_wt.%s.fst", chr), compress = 100)
        TRUE
    }
    data_hdf5$close_all()
...
data_path <- "../wu2016_wt_no_chunk/output/tasks/kinetics_tools.tasks.ipd_summary-0/basemods.h5"
wu2016_wt_data <- parse_kinetics_h5(data_path, chrs)
cat("Loaded data wu2016_wt\n")
